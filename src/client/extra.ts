...
const extra2 = function () {
  ...
  const editorEvaluate = function () {
	...
	*/
      ...
      myshell.postMessage(s, false); // important not to move the pointer so can move to next line
      // s.split("\n").forEach((line) => myshell.postMessage(line, false)); // should work fine now that echo mode is on but not needed
