...
function baselinePosition(el) {
  ...
  fs0.appendChild(document.createTextNode("X"));
  ...
  el.appendChild(fs0);
  const result =
    fs0.getBoundingClientRect().top - fs1.getBoundingClientRect().top;
  el.removeChild(fs0);
  el.removeChild(fs1);
  return result;
}
...
const addToEl = function (el, pos, s) {
...
const placeCaretAtEnd = function (el, flag?) {
  // flag means only do it if not already in input. returns position
  if (!flag || document.activeElement != el) {
    el.focus();
    placeCaret(el, el.textContent.length);
  }
};
const attachEl = function (el, container) {
...
const Shell = function (
  ...
  const removeAutoComplete = function (flag) {
    ...
    if (autoComplete) {
      ...
      else addToEl(inputSpan, pos, autoComplete.dataset.word);
      autoComplete.remove();
  ...
  const escapeKeyHandling = function (pos) {
    ...
    if (esc < 0) addToEl(inputSpan, pos, "\u250B");
    else {
      var s;
      if (esc < pos) {
  ...
  const tabKeyHandling = function (pos) {
    ...
    if (j < M2symbols.length) {
      ...
      while (
        k < M2symbols.length &&
        M2symbols[k].substring(0, word.length) == word
      )
        k++;
      if (k > j) {
        if (k == j + 1) {
          ...
          addToEl(
            inputSpan,
            pos,
            M2symbols[j].substring(word.length, M2symbols[j].length) + " "
          );
        } else {
  ...
  var mytranspose = (m) =>
    m[0].map((col, i) => {
      var a = m.map((row) => row[i]);
      (a as any).type = "katexlist";
      return a;
    });
  function stringM2(x) {
    ...
    else if (x.type === "Subscript" && x.length > 2) {
      var a = x.slice(1);
      (a as any).type = "Sequence";
      x = [x[0], a];
    }
  ...
  function createM2(x, flag, vlistflag) {
    // reproduces a first approximation of the expression tree of M2
    var a = [];
    ...
    if (vlistflag && x.classList.contains("vlist-t"))
      (a as any).type = "katexlist"; // special case
    ...
    for (var i = 0; i < x.children.length; i++) {
      var s = createM2(
        ...
        (!x.classList.contains("M2Meta") && vlistflag) ||
          x.dataset.type === "MatrixExpression" ||
          x.dataset.type === "Table"
      );
  ...
  const closeHtml = function () {
    ...
    if (htmlSec.classList.contains("M2Script")) {
      ...
      try {
        anc.innerHTML = anc.dataset.saveHTML += katex.renderToString(
    ...
    } else if (anc.classList.contains("M2Html")) {
      // we need to convert to string
      anc.innerHTML = anc.dataset.saveHTML += htmlSec.outerHTML;
    } else if (anc.classList.contains("M2Latex")) {
      // *try* to convert to texcode TEMP gotta replace this with more serious?
      var fontSize: number = +window
      ...
      anc.dataset.texCode +=
        ...
        baseline / fontSize +
        "}{" +
        (htmlSec.offsetHeight - baseline) / fontSize +
        "}{" +
        htmlSec.outerHTML +
        "}}";
  ...
  const closeInput = function () {
    ...
    if (inputSpanParentElement.length > 0)
      attachEl(inputSpan, inputSpanParentElement.pop());
