	...
	// Forward key for tab completion, but do not track it.
	  if (e.keyCode === keys.tab) {
		  ...
		  if (k>j) {
			  ...
			  tabMenu.setAttribute("tabindex","0"); // TEMP hack
			  for (var l=j; l<k; l++)
			  {
			      var opt = document.createElement("li");
			      opt.textContent=M2symbols[l];
			      opt.addEventListener("click", function() { // the event listener should be on the whole menu, with a notion of currently selected whose css would replace the current hover. (there should also be a onmouseover event)
			  ...
			  shell[0].appendChild(tabMenu); // if we're gonna use inline-table, then should add extra element with " "+rest of text probably all inside a <span></span> to remove easier
			  tabMenu.focus();
			  tabMenu.addEventListener("keydown", function(e) { e.stopPropagation(); return true; }); // unfinished: should let user go up/down and select with enter. any other key should propagate
