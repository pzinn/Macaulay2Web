...
function addToInput(s) { inputDiv.textContent+=s; }
...
*/ // TODO rewrite. for now CTRL-C is usual "copy"
	...
	// Forward key for tab completion, but do not track it.
	  if (e.keyCode === keys.tab) {
	      var sel = window.getSelection();
	      var pos = sel.extentOffset; // check with firefox. note that inputDiv should always have *one text node*
		  ...
		  if (k>j) {
			  ...
			  inputDiv.textContent = msg.substring(0,i+1)+M2symbols[j]+" "+msg.substring(pos,msg.length);
			  // put the caret where it should be
			  sel.collapse(sel.extentNode,i+2+M2symbols[j].length);
		      }
			  ...
			  for (var l=j; l<k; l++)
			  {
				  ...
				  addToInput(this.textContent.substring(word.length,this.textContent.length)+" ");
			      });
